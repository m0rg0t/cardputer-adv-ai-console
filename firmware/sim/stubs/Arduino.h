...
// UI units touch is provided; everything else is intentionally absent.
#pragma once
...
#define millis() cardputer_sim::simMillis()
#define delay(ms) cardputer_sim::simDelay(ms)
...
public:
    ...
    String(float number, int decimals = 2)
    {
        char text[32];
        snprintf(text, sizeof(text), "%.*f", decimals, number);
        value_ = text;
    }
    ...
    {
        return String(a.value_ + (b ? b : ""));
    }
    friend String operator+(const char* a, const String& b)
    {
        return String(std::string(a ? a : "") + b.value_);
    }
    friend String operator+(const String& a, char b)
    {
        return String(a.value_ + b);
    }
    ...
    int indexOf(const String& text, std::size_t from = 0) const
    {
        return indexOf(text.c_str(), from);
    }
    bool startsWith(const char* prefix) const
    {
        return value_.rfind(prefix ? prefix : "", 0) == 0;
    }
    ...
    bool endsWith(const char* suffix) const
    {
        ...
        return value_.size() >= tail.size() &&
               value_.compare(value_.size() - tail.size(), tail.size(),
                              tail) == 0;
    }
    ...
    void replace(const String& from, const String& to)
    {
        ...
        while ((position = value_.find(from.value_, position)) !=
               std::string::npos) {
            value_.replace(position, from.length(), to.value_);
            position += to.length();
        }
    }
